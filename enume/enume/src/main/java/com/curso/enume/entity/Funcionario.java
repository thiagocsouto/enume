...
//	private Date dataSaida;
	...
	public Funcionario(
			...
			@Pattern(regexp = "^\\d{1,3}(\\.\\d{3})*,\\d{2}$", message = "Digite um salário válido.") String salarioBruto,
			@Pattern(regexp = "^[a-zA-ZáàâãéèêíïóôõöúçñÁÀÂÃÉÈÊÍÏÓÒÖÚÇÑ ]*$", message = "naõ deve conter número ou caractere especial.") String cidade,
			@Pattern(regexp = "^[a-zA-ZáàâãéèêíïóôõöúçñÁÀÂÃÉÈÊÍÏÓÒÖÚÇÑ ]*$", message = "naõ deve conter número ou caractere especial.") String logradouro,
			@Pattern(regexp = "\\d+", message = "Somente números válidos no campo.") String numero, String complemento,
			String bairro, String uf,
			@Pattern(regexp = "\\d{5}-\\d{3}", message = "deve corresponder ao exemplo: 00000-000") String cep) {
		super();
		this.nome = nome;
		this.cargo = cargo;
